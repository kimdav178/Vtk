...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth = np.power(self.nodes[0, :], 0)
        #self.smth = 1000*np.arange(int(len(nodes_coords) / 3))
    ...
    def move(self, tau):
        ...
        self.velocity[2] = -(self.nodes[0]+5.65)     # Центр находится около -5.65 по координате х
        # По сути метод просто двигает все точки c их текущими скоростями
        self.nodes += self.velocity * tau

    def cardio(self, intensity, width, a, tau):
        for i in range(len(self.nodes[0, :])):
            x = self.nodes[0, i]
            y = self.nodes[1, i] + 5
            if ((x * x + y * y + 2 * a * y) ** 2 - 4 * a * a * (x * x + y * y)) < width:
                self.smth[i] += tau * intensity
